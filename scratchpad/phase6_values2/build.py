"""Convert phase6_values2/*.txt compact files into authored/g5_p6_values2.jsonl and validate.
...
BANNED = re.compile(r"\b(sex\w*|nude|naked|porn\w*|politic\w*|democrat\w*|republican\w*|liberal|conservative|election|vote|voting|diagnos\w*|disorder|adhd|autis\w*|bipolar|ocd|ptsd|depression|depressed|suicid\w*|self-harm|medication|psychiatr\w*|trump|biden|abortion|immigra\w*|death penalty|capital punishment|execution|gun control|affirmative action|climate policy|carbon tax|partisan|left-wing|right-wing|erotic|lover)\b", re.I)
...
for p in files:
    ...
    for i, line in enumerate(p.read_text().splitlines(), 1):
        ...
        if prim == "choice":
            ...
            opts = None
        out.append({"text": text, "primitive": prim, "options": opts, "node": node, "kind": "values", "_w": where})

existing = []
for f in ROOT.glob("authored/*.jsonl"):
    if f.name == "g5_p6_values2.jsonl": continue
...
if "--fuzzy" in sys.argv:
    # also compare against the other values2 txt files so forks see each other's work
    others = []
    for f in D.glob("*.txt"):
...
if "--write" in sys.argv:
    ...
    with open(ROOT / "authored/g5_p6_values2.jsonl", "w") as f:
        for q in out:
            q.pop("_w"); f.write(json.dumps(q, ensure_ascii=False) + "\n")
    print("wrote authored/g5_p6_values2.jsonl")
