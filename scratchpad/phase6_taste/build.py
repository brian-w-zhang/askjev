"""Convert phase6_taste/<node_id>.txt compact files into authored/g5_p6_taste.jsonl and validate.
...
for f in sorted((ROOT / "authored").glob("*.jsonl")):
    if f.name in ("g5_p6_taste.jsonl",) or "routing" in f.name or "search" in f.name: continue
...
if "--write" in args and not errs:
    with open(ROOT / "authored/g5_p6_taste.jsonl", "w") as f:
        for q in out: f.write(json.dumps(q, ensure_ascii=False) + "\n")
    print("wrote authored/g5_p6_taste.jsonl")
